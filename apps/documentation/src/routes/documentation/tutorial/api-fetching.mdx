...
To start, let’s fetch all of them from the root page. Since we want to render them on the server side, we going to need to implement the logic in the `index.rs` file.
